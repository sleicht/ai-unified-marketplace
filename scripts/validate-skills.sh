...
if rg -n 'localhost|DEFAULT_POC_EMPLOYEE_TOKEN' \
  aiup-compose-ktor-exposed/skills/aiup-compose-test/references/ExampleScreenTest.kt; then
  echo "Stale UI client example found" >&2
  exit 1
fi

if rg -n 'ServiceApiClient\(\)' \
  aiup-compose-ktor-exposed/skills/aiup-implement-ui/SKILL.md \
  aiup-compose-ktor-exposed/skills/aiup-implement-ui/references/ui-style.md; then
  echo "Stale UI client example found" >&2
  exit 1
fi

if rg -n 'accessTokenProvider\.accessToken\(\)|override suspend fun accessToken\(' \
  aiup-compose-ktor-exposed/skills/aiup-implement-ui \
  aiup-compose-ktor-exposed/skills/aiup-compose-test/references \
